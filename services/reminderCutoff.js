        ...
        for (const row of missingRes.rows) {
            ...
            if (process.env.ENGAGE_ENABLED === 'true') {
                ...
                const smsBody = pending === 1
                    ? `⏰ En ${minutesLeft} min cierra ${t.tournament_name}. 1 pronóstico sin cargar 👉 hr.prodecaballito.com/apuestas`
                    : `⏰ En ${minutesLeft} min cierra ${t.tournament_name}. Te faltan ${pending} pronósticos 👉 hr.prodecaballito.com/apuestas`;
    ...
    for (const match of standaloneRes.rows) {
        ...
        for (const row of missingRes.rows) {
            ...
            if (process.env.ENGAGE_ENABLED === 'true') {
                ...
                const smsBody = `⏰ ${match.home_team} vs ${match.away_team} cierra en ${minutesLeft} min — aún no pronosticaste 👉 hr.prodecaballito.com/apuestas`;
                await sendSMSWithRetry({ to: row.whatsapp_number, body: smsBody })
                    .catch(err => console.error(`[cutoff-reminder] sms failed (after retries) user=${row.user_id}:`, err.message));
            }
