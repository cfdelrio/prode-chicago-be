...
router.post('/forgot-password', rateLimit_1.authLimiter, async (req, res) => {
    try {
        ...
        const resetLink = `${process.env.FRONTEND_URL || 'https://hr.prodecaballito.com'}/recuperar-contrasena?token=${resetToken}`;
        console.log(`📧 Enviando email de recuperación a ${email}`);
        try {
            await (0, email_1.sendEmail)({
                to: email,
                subject: '🔐 Recupera tu contraseña — PRODE High Rolling',
                html: `
                    <div style="font-family:system-ui,sans-serif;max-width:600px;margin:0 auto;background:#f8fafc">
                        ...
                        <p style="text-align:center;color:#9ca3af;font-size:0.75rem;padding:16px">
                            PRODE High Rolling · <a href="https://hr.prodecaballito.com" style="color:#0042A5">hr.prodecaballito.com</a>
                        </p>
                    </div>
                `,
            });
